...
# Usernames generated by deploy-verification scripts (never real accounts).
_TEST_USERNAME_PATTERNS = ("hacker_admin_%", "normal_doc_%", "smoke_%", "e2e_%",
                           "deploy_check_%", "prune_admin", "dr_live_check", "ok_%")


def delete_test_users() -> dict:
    """Remove leftover verification accounts (by username pattern) and clear the
    now-empty data tables. Scoped -- does not touch real accounts."""
    ...
    with _engine().begin() as conn:
        ...
        cond = or_(*[schema.users.c.username.like(p) for p in _TEST_USERNAME_PATTERNS])
        counts["users"] = conn.execute(schema.users.delete().where(cond)).rowcount
    return counts
