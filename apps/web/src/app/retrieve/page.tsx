...
export default function RetrievePage() {
  ...
  const handleRetrieve = async () => {
    ...
    try {
      const res = await api.retrieve(patientId, purpose, consent);
      setResult(res);
  ...
  if (session.ready && !session.loggedIn) {
    return <SignInPrompt action="retrieve patient data" />;
  }
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      <motion.div variants={item}>
        ...
        <p className="text-surface-500 mt-1">
          Access patient data with context-aware masking, as{' '}
          <span className="font-semibold capitalize text-primary-600">{session.role || '...'}</span>.
        </p>
      </motion.div>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <motion.div variants={item} className="lg:col-span-2 glass-card-solid p-6 space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-surface-700 mb-2">Your Role</label>
            <div className="px-3 py-2 rounded-lg bg-primary-50 text-primary-700 border border-primary-200 text-sm font-medium capitalize inline-flex items-center gap-2">
              <Shield className="w-4 h-4" /> {session.role || 'unknown'}
              <span className="text-xs text-primary-400 font-normal">(from your sign-in)</span>
            </div>
          </div>
